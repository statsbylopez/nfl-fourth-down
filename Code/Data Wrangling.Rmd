...
*Note*: `plays` and `games` data from Armchair Analysis, while `ratingswinprob` from Football Outsiders
...
Implement random forest win probability model of Lock and Nettleton (JQAS, 2014)
...
Implement version of generalized additive model used by Horowitz et al with `nflscrapr` package in R
...
Here, we make an accuracy plot of the binned win probabilities for each of the two models. 

```{r}
temp.wp <- NFLwin %>%
  select(gid, pid, wp.Scrapr, wp.Lock, Home) %>% 
  mutate(wp.ave = (wp.Lock + wp.Scrapr)/2, 
        wp.home.ave = ifelse(Home == 1, wp.ave,  1 - wp.ave)) %>%  
  group_by(gid) %>% 
  mutate(wp.home.lead.ave = lead(as.numeric(wp.home.ave), 1), 
         deltawp.home.ave = wp.home.lead.ave - wp.home.ave) %>% 
  na.omit(wp.home.lead.ave)  %>% #Remove plays at the end of the game where there is no next play.
  mutate(wp.off.lead.ave = ifelse(Home == 1, wp.home.lead.ave, 1 - wp.home.lead.ave),
         deltawp.off.ave = wp.off.lead.ave - wp.ave)


wp.l.p <- agg.Lock.reg %>% 
  mutate(wp.model = "Lock") %>% 
  select(category, mean.win, wp.model)

wp.s.p <- agg.NFLS.reg %>% 
  mutate(wp.model = "NFLscrapR") %>%  
  rename(mean.win = mean.win.s) %>% 
  select(category, mean.win, wp.model)

wp.plot.data <- bind_rows(wp.l.p, wp.s.p)


p.wp.test <- ggplot(wp.plot.data, aes(x = category, y = ))
p.wpacc <- ggplot(wp.plot.data, aes(x = category, y = mean.win, colour = wp.model, shape= wp.model))
wpacc <- p.wpacc + 
  geom_point() + 
  geom_abline(intercept = -.025, slope = 1/20) + 
  scale_x_continuous(name = "Estimated Win Probability",
...
wpacc
#ggsave(wpacc, file = "Figures/WP_accuracy_plot.png")
```
